...
-- Revertir: quitar search_vector y prov_des, restaurar cat_des con datos desde cat_art.
DROP INDEX IF EXISTS idx_art_search_vector;
ALTER TABLE art DROP COLUMN IF EXISTS search_vector;

ALTER TABLE art DROP COLUMN IF EXISTS prov_des;

ALTER TABLE art ADD COLUMN IF NOT EXISTS cat_des TEXT NOT NULL DEFAULT '';

UPDATE art a
SET cat_des = COALESCE(c.cat_des, '')
FROM cat_art c
WHERE a.co_cat = c.co_cat;
