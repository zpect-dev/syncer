...
func (r *SourceRepository) FetchArticlesPage(ctx context.Context, limit, offset int) ([]Article, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&item.TipoImp, &item.CoLin, &item.CoCat, &item.CoSubl, &item.Campo4, &item.SinDsc, &item.CoColor, &item.CoProv, &item.ProvDes, &item.Garantia, &item.Ref,
		); err != nil {
	...
	for i := 0; i < len(items); i += batchSize {
		...
		for _, item := range chunk {
			args = append(args,
				...
				item.PrecVta1, item.PrecVta2, item.PrecVta3, item.PrecVta4, item.PrecVta5,
				item.TipoImp,
				toNull(item.CoLin), toNull(item.CoCat), toNull(item.CoSubl), toNull(item.Campo4), item.SinDsc, toNull(item.CoColor), toNull(item.CoProv), item.ProvDes, toNull(item.Garantia), item.Ref,
				// item.ImageURL,
			)
		}

		queryTpl := `
			INSERT INTO art (co_art, art_des, prec_vta1, prec_vta2, prec_vta3, prec_vta4, prec_vta5, tipo_imp, co_lin, co_cat, co_subl, campo4, sin_dsc, co_color, co_prov, prov_des, garantia, ref)
			VALUES %s
